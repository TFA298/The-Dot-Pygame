...
pygame.display.set_caption("The Dot #8")
...
def run_stage():
    ...
    while run:
        ...
        if len(boxes) < random_boxes :
            for i, timer in enumerate(respawn_timer):
                respawn_timer[i] -= 1
                if timer <= 0:
                    boxes.append(random_object())
                    respawn_timer.remove(respawn_timer[i])
        ...
        for box in boxes:
            if circle_object.colliderect(box) and not collision_occured:
                scores += 1
                boxes.remove(box)
                respawn_timer.append(180)
                collision_occured = True

        score_couter(f"Score : {scores}", text_font, "black", 300, 25)
